...
% Cross-references into the supplementary appendix are hardcoded (e.g.
% "Appendix~A.2") rather than resolved via xr, so the main paper compiles
% standalone on Overleaf without needing supplementary.aux.
...
Central to our approach is a controlled decoding algorithm, $\beta$-Hindsight control, which uses latent randomness as a tunable similarity control mechanism during counterfactual generation.
...
\begin{figure}[!t]
    ...
    \caption{Overview of counterfactual student writing generation. Our approach produces a improved version of student work that remains similar to the original.}
...
In education, however, student work is often scored in an ordinal way. Therefore, counterfactual work must reflect nuanced adjustments in student work on a quality spectrum.
...
\bottomrule
\end{tabular}
\caption{Comparing GM to other counterfactual methods on both datasets (ME - Minimal-Edit, ICL - In-Context Learning, I\&R - Identify-and-Replace, GM - Gumbel Machine).}
...
% \multirow{-2}{*}{\textbf{Approach}} & $\text{Sim}^\uparrow$ & $\text{Val}^\uparrow$ & $\text{Sim}^\uparrow$ & $\text{Val}^\uparrow$ \\
% \midrule
% ME (FT) %Minimal-Edit, fine-tuned
%     & $0.324$ & $\textbf{0.690}$
%     & $0.224$ & $\textbf{0.607}$ \\
% ICL (FT)
%     & $0.282$ & $0.316$
%     & $0.268$ & $\underline{0.426}$ \\
% I\&R (FT) %Identify-and-Replace, fine-tuned
%     & $\textbf{0.461}$ & $0.392$
%     & $\underline{0.359}$ & $0.175$ \\
% GM (Ours)
%     & $\underline{0.391}$ & $\underline{0.668}$
%     & $\textbf{0.477}$ & $0.404$ \\
% \bottomrule
% \end{tabular}
% \caption{\mh{FT}Comparing GM to other counterfactual generation methods on both datasets (ME - Minimal-Edit, ICL - In-Context Learning, I\&R - Identify-and-Replace, GM - Gumbel Machine).}
% \label{tab:llama_results}
% \vspace{-.3cm}
\end{table}
...
\textbf{Minimal-Edit (ME)}: Following \citet{gat_faithful_2023}, we include in the prompt instructions for the LLM to minimally edit the reference, which is also done in GM, as a zero-shot prompting approach.
\textbf{In-Context Learning (ICL)}: Following \citet{li_prompting_2024}, we further include in-context demonstrations atop ME for each score level in the prompt.
...
We now detail experimental results and perform both human evaluation and ablation studies to verify our approach. 
...
\subsection{Gumbel Noise: A Flexible Control Signal} \label{sec:tuning}

In Figure~\ref{fig:pareto-front}, we report the results of our experiments to tune the controllable $\beta$ parameter in the GM. In these experiments, we vary the control signal across a range of values for GM and compare with Vocab Bias, a simple decoding strategy that adds a tunable $\alpha$ constant to all logits for all tokens in the reference summary \cite{yang_fudge_2021}. For GM, higher values of $\beta$ emphasize similarity to the original student response, while lower values prioritize satisfying the desired target score. For Vocab Bias, higher values of $\alpha$ induce stronger similarity to reference tokens. We sweep the control parameters across the values $\beta \in \{0.001, 0.1, 0.2, 0.3, 0.4, 1.0\}$, $\alpha \in \{0.01, 5.0, 10.0\}$ for CLASSE and $\beta \in \{0.05, 0.2, 0.4, 0,5, 0.6, 0.7, 0.8, 1.0\}$, $\alpha \in \{0.01, 1.0, 5.0\}$ for DREsS. We observe that values outside these ranges result in collapses in model fluency. % due to excessive logit manipulation.

Comparing the two approaches, we observe that for GM, increasing values of $\beta$ enable a trade-off between similarity that can explore a much wider range of the Pareto front compared to Vocab Bias. Further, we observe that GM tends to maintain higher validity with increasing similarity values. These observations suggest that recovered Gumbel noise provides a more effective control signal for generating similar counterfactual randomness than using token-level heuristics or prompting techniques for similarity control. 
% This finding suggests that similarity control in LLMs is more robust when the entire sampling trajectory and random states are considered rather than solely token-level alignment.

% Individual curve plots stacked in a SINGLE column. Each panel gets the full
% (shrunk) column width, so it reads larger than the combined image's cramped
% half-panels; single-column placement displaces only one column, giving the
% least content overflow of the readable layouts (10 pp). Tune 0.75\linewidth
% to taste (larger -> more readable but eventually fills the whole column).
% \begin{figure}[t]
%     \centering
%     \includegraphics[width=0.75\linewidth]{tradeoff_classe_curve.png}\\[2pt]
%     \includegraphics[width=0.75\linewidth]{tradeoff_dress_curve.png}
%     \caption{Similarity–validity trade-offs comparing Gumbel Machine with a logit bias (Vocab Bias) and baseline approaches (ME - Minimal-Edit, ICL - In-Context Learning, I\&R - Identify-and-Replace) on CLASSE (top) and DREsS (bottom). Gumbel Machine provides a tunable tradeoff between competing metrics, occupying a different region of the Pareto frontier.}
%     \label{fig:pareto-front}
% \end{figure}
\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\linewidth]{latex/figs/stacked_curve_v2.png}\\[2pt]
    \caption{Similarity–validity trade-offs comparing Gumbel Machine with logit bias (Vocab Bias) and prompting baseline approaches (ME - Minimal-Edit, ICL - In-Context Learning, I\&R - Identify-and-Replace). Gumbel Machine provides a tunable tradeoff between competing metrics, on a different region of the Pareto frontier.}
    \label{fig:pareto-front}
    \vspace{-.3cm}
\end{figure}
